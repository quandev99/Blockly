...
Blockly.defineBlocksWithJsonArray([
  ...
  {
    type: "set_attribute",
    message0: "set the1 attribute %1 to %2",
    args0: [
      {
        type: "field_dropdown",
        name: "PROPERTY",
...
function renameLocalVariablesUsedInBlock(block) {
  ...
  renamedVariables.forEach((variable) => {
    variable.blocklyVariable = BlocklyJavaScript.nameDB_.variableMap_.getVariableById(
      variable.id
    );
...
Blockly.Blocks["controls_switch_case"] = {
  init: function () {
    this.appendValueInput("SWITCH_VALUE").setCheck(null).appendField("switch");
    this.setColour(230);
    this.setTooltip("");
    this.setHelpUrl("");
    this.setPreviousStatement(true);
    ...
    this.setMutator(new Blockly.Mutator(["case_emulator", "default_emulator"]));
  },

  saveExtraState: function () {
    const state = {
      casesCount: this.casesCount_,
  ...
  loadExtraState: function (state) {
    this.casesCount_ = state.casesCount || 0;
    this.hasDefault_ = state.hasDefault || false;
    this.updateShape_();
  },

  decompose: function (workspace) {
    var containerBlock = workspace.newBlock("switch_emulator");
    containerBlock.initSvg();
    var connection = containerBlock.getInput("STACK").connection;
    for (var i = 0; i < this.casesCount_; i++) {
      var caseBlock = workspace.newBlock("case_emulator");
      caseBlock.initSvg();
      connection.connect(caseBlock.previousConnection);
      connection = caseBlock.nextConnection;
    }
    if (this.hasDefault_) {
      var defaultBlock = workspace.newBlock("default_emulator");
  ...
  compose: function (containerBlock) {
    ...
    this.updateShape_();
    for (var i = 0; i < this.casesCount_; i++) {
      Blockly.Mutator.reconnect(valueConnections[i], this, "CASE_VALUE" + i);
  ...
  saveConnections: function (containerBlock) {
    ...
    while (clauseBlock) {
      switch (clauseBlock.type) {
        case "case_emulator":
          var inputCase = this.getInput("CASE_VALUE" + i);
          clauseBlock.valueConnection_ =
            inputCase && inputCase.connection.targetConnection;
          var inputDo = this.getInput("CASE_DO" + i);
          clauseBlock.statementConnection_ =
            inputDo && inputDo.connection.targetConnection;
          i++;
          break;
        case "default_emulator":
          var inputDefault = this.getInput("DEFAULT_DO");
          clauseBlock.statementConnection_ =
            inputDefault && inputDefault.connection.targetConnection;
          break;
        default:
          throw "Unknown block type.";
      }
  ...
  updateShape_: function () {
    // Delete everything.
    if (this.getInput("DEFAULT_DO")) {
      this.removeInput("DEFAULT_DO");
    }
    var i = 0;
    while (this.getInput("CASE_VALUE" + i)) {
      this.removeInput("CASE_VALUE" + i);
    ...
    // Rebuild block.
    for (var i = 0; i < this.casesCount_; i++) {
      this.appendValueInput("CASE_VALUE" + i)
        .setCheck(null)
        .appendField("case");
...
Blockly.Blocks["switch_emulator"] = {
  init: function () {
...
Blockly.Blocks["default_emulator"] = {
  init: function () {
...
Blockly.JavaScript["controls_switch_case"] = function (block) {
  console.log("block", block);
  var switchValue1 = Blockly.JavaScript.valueToCode(
    block,
    "SWITCH_VALUE",
    Blockly.JavaScript.ORDER_ATOMIC
  );
  var code = "switch (" + `${switchValue1 ? switchValue1 : '""'}` + ") {\n";
  // if(block?.casesCount_ <= 0){
  //    if (block.hasDefault_) {
  //      var defaultCode = Blockly.JavaScript.statementToCode(
  //        block,
  //        "DEFAULT_VALUE"
  //      );
  //      code += "default:\n" + defaultCode;
  ...
  //   return code;
  // }else{
  for (let i = 0; i < block.casesCount_; i++) {
    var caseValueI = Blockly.JavaScript.valueToCode(
      block,
      "CASE_VALUE" + i,
      Blockly.JavaScript.ORDER_ATOMIC
    );
    var statements_do_I = Blockly.JavaScript.statementToCode(
      block,
      "CASE_DO" + i
    );
    code += " case " + `${caseValueI ? caseValueI : '""'}` + ":\n" + statements_do_I + " break;\n";
  }
  if (block.hasDefault_) {
    var defaultCode = Blockly.JavaScript.statementToCode(block, "DEFAULT_DO");
    code += " default:\n" + defaultCode;
  }
  code += "}\n ";
